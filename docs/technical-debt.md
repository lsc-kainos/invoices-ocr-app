...
> Documento vivo. Problemas médios de arquitetura, código e infraestrutura identificados na revisão técnica de 2026-05-11. **Não são bugs críticos** — não impedem o funcionamento do sistema — mas devem ser endereçados para garantir escalabilidade e manutenibilidade da codebase.
...
**Sugestão:** Extrair todas as páginas e processar como array de imagens, ou concatenar verticalmente.
...
**Impacto:** Quebra lifecycle hooks do Nest (onModuleInit, onModuleDestroy). Não há injeção de dependências nos providers.  
...
**Sugestão:** Criar `LlmProviderFactory` injetável que encapsula a lógica de criação.

---

### BT-05: `AppModule` acessa `process.env` diretamente

**Impacto:** Inconsistência com `ConfigService`. `NODE_ENV` não é substituível em testes de unidade.  
...
**Impacto:** Carrega todas as mensagens em `desc`, faz `reverse()` O(n), depois `findIndex` O(n).  
...
**Sugestão:** Transaction compensatória (cleanup async) ou usar transação distribuída.

---

### BT-10: `signFileUrl` retorna path relativo, não URL completa

**Impacto:** Incompleto se houver CDN ou domínio diferente.  
**Arquivo:** `apps/api/src/documents/documents.service.ts`  
**Sugestão:** Adicionar `BASE_URL` ao path ou retornar objeto `{ path, baseUrl }`.
...
**Sugestão:** Usar `crypto.randomUUID()` ou `Math.random()`.
...
- Itens marcados como "débito técnico" são **deliberadamente** deixados para depois do core, conforme a estratégia do projeto.
